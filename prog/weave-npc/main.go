...
func createBaseRules(ipt *iptables.IPTables, ips ipset.Interface) error {
	...
	if err := ipt.Append(npc.TableFilter, npc.EgressChain,
		"-m", "set", "!", "--match-set", npc.LocalIpset, "src", "-j", "RETURN"); err != nil {
		return err
	}
	if err := ipt.Append(npc.TableFilter, npc.EgressChain,
		"-m", "state", "--state", "NEW", "-g", string(npc.EgressDefaultChain)); err != nil {
		return err
	}
	if err := ipt.Append(npc.TableFilter, npc.EgressChain,
		"-m", "state", "--state", "NEW", "-g", string(npc.EgressCustomChain)); err != nil {
		return err
	}
	if err := ipt.Append(npc.TableFilter, npc.EgressChain,
	...
	if err := ipt.Append(npc.TableFilter, npc.EgressChain, "-j", "DROP"); err != nil {
		return err
	}
